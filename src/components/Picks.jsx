...
export default function Picks({ session, activeSport }) {
  ...
  const [units, setUnits] = useState(Object.fromEntries(PICK_CATS.map(c => [c.id, 15])))
  ...
  async function loadPicks(poolEntryId) {
    ...
    if (data) {
      const pickMap = {}
      const unitMap = { ...Object.fromEntries(PICK_CATS.map(c => [c.id, 15])) }
      data.forEach(p => {
  ...
  async function savePick(catId, gameId, team, lockedOdds) {
    ...
    const existing = await supabase
      .from('picks')
      .select('id')
      .eq('pool_entry_id', activePoolEntry.id)
      .eq('category', catId)
      .single()
  ...
  async function saveUnits(catId, val) {
    const newUnits = { ...units, [catId]: Math.max(1, Math.min(70, parseInt(val) || 1)) }
    setUnits(newUnits)
    if (picks[catId] && activePoolEntry) {
      await supabase.from('picks')
        .update({ units: newUnits[catId], updated_at: new Date().toISOString() })
        .eq('pool_entry_id', activePoolEntry.id)
        .eq('category', catId)
    }
  }
  ...
  return (
    <div>
      ...
      {myPools.length > 1 && (
        <div style={s.poolSelector}>
          ...
          <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
            {myPools.map(entry => (
              <div key={entry.id}
                style={{...s.poolChip, ...(activePoolEntry?.id===entry.id ? s.poolChipActive : {})}}
      ...
      <div style={s.unitsPanel}>
        ...
        <div style={{flex:1}}>
          <div style={s.unitsLabel}>Units Allocated — {totalUnits}%</div>
          ...
          <div style={s.unitsSub}>{remaining > 0 ? `${remaining} units left` : remaining === 0 ? '✅ All 100 units allocated' : '⚠️ Over budget — reduce units'}</div>
        </div>
      </div>
      ...
      <div style={s.picksGrid}>
        {PICK_CATS.map(cat => {
          ...
          return (
            <div key={cat.id} style={{...s.pickCard, borderColor: pick ? cat.color : '#e2dfd8', opacity: locked ? 0.75 : 1}}
              ...
              <div style={s.pickCardFooter} onClick={e => e.stopPropagation()}>
                <button style={s.unitBtn} disabled={locked} onClick={e => { e.stopPropagation(); saveUnits(cat.id, units[cat.id] - 1) }}>−</button>
                <div style={{flex:1}}>
                  <input style={{...s.unitInput, color: cat.color}} type="number" min="1" max="70"
                    value={units[cat.id]} disabled={locked}
                    onChange={e => saveUnits(cat.id, e.target.value)} />
        ...
        <div style={s.modalOverlay} onClick={() => setOpenModal(null)}>
          <div style={s.modalBox} onClick={e => e.stopPropagation()}>
            <div style={s.modalHeader}>
              <div>
                <div style={{...s.modalTag, background: PICK_CATS.find(c=>c.id===openModal)?.color}}>{PICK_CATS.find(c=>c.id===openModal)?.label}</div>
            ...
            <div style={{padding:'14px 16px 24px'}}>
              {games.map(game => {
                ...
                return (
                  <div key={game.id}
                    style={{...s.modalGame, borderColor: isSelected ? PICK_CATS.find(c=>c.id===openModal)?.color : '#e2dfd8',
                      background: isSelected ? '#f0eaf9' : '#fff', opacity: game.started ? 0.45 : 1,
                      cursor: game.started ? 'not-allowed' : 'pointer'}}
...
const s = {
  ...
  unitInput:{width:'100%',textAlign:'center',fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:'1.2rem',border:'1.5px solid #e2dfd8',borderRadius:'8px',padding:'3px 8px',background:'#fff',outline:'none'},
